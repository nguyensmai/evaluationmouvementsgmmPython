...
from functions.estimateOrientationFromPosition import *
from functions.removeStart import *
import scipy.signal
...
def loadData(dir, fname, filt, est, rem, ws, resampleSize):
    ...
    with open(path, 'r+', encoding='utf-8') as f:
        M = np.array([i[:-2].split(' ') for i in f.readlines()]).astype(float)  ##if we use i[:-1], there will be a '' at the end of each line
    oriMat = M[:,3:7]
    posMat = M[:,0:3]
    for j in range(1,25):
        fO = j*7 + 3
